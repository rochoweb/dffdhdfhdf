<?php
	...
	class CHR {
		...
		public function CHECK_SUBPAGE() {
			...
			$stmt = $this->m->q("SELECT * FROM pages WHERE name = '$page'");
		...
		public function CHECK_LANG($page) {
			...
			$stmt = $this->m->q("SELECT lang FROM pages WHERE name = '$page'");
			$r = $stmt->fetch_object();
		...
		public function PAGEDATA($PID, $lang) {
			...
			$stmt = $this->m->q("SELECT * FROM pages WHERE page = '$PID' AND lang = '$lang'");
			$r = $stmt->fetch_assoc();
		...
		public function PAGEDATA_DEFAULT() {
			...
			$stmt1 = $this->m->q("SELECT * FROM pages WHERE name = '$page'");

			if ( !$r = $stmt1->fetch_object() ) {

				$stmt2 = $this->m->q("SELECT * FROM navody WHERE url = '$page'");
				if ( $stmt2->fetch_object() ) {
					$stmt3 = $this->m->q("SELECT * FROM pages WHERE page = 16");
				...
				if ( !$r ) {
					...
					$stmt4 = $this->m->q("SELECT * FROM kategorie WHERE sk = '$page'");
					if ( $stmt4->fetch_object() ) {
						$stmt5 = $this->m->q("SELECT * FROM pages WHERE page = 5");
		...
		public function PAGEDATA_TARGET($pageID = "", $r ="") {
			...
			$stmt = $this->m->q("SELECT * FROM pages WHERE ".$select." AND lang = '".$this->LANG."'");
		...
		public function ITEM_INFO($table, $identificator, $item) {
			...
			$stmt = $this->m->q("SELECT * FROM $table WHERE $identificator = '$item'");
		...
		public function USERDATA( $data = "" ) {
			if ( $data ) {
				...
				if ( $this->online() ) {
					...
					//$shop = $this->m->q("SELECT * FROM eshops WHERE ownerID = ".$user["user"]);
