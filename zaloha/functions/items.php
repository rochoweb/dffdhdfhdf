			...
			switch ($page) {
				...
				case "itemrelated":
					...
					if ( !$related = $this->mysql->resultO() ) {
						$stmt2 = $this->mysql->query("SELECT * FROM navody WHERE public = 1 AND availability != 8 AND availability != 0 AND id != '".$DATA["id"]."' ORDER BY RAND() LIMIT ".ITEM_RELATED);
						$R = $this->mysql->resultO();
					} else {
						$R = $related;
					}
				...
				case 'category':
					if ( isset($_GET["filter"]) ) {
						...
						$this->mysql->query("SELECT * FROM navody WHERE public = 1 AND category = '".$DATA->category."' ORDER BY create_date DESC LIMIT ".$oddo["od"].", ".$oddo["do"]);
