		...
		public function order_data($r = '') {
			...
			if ( $d = $this->GD->basket_data( $this->basket ) ) {
				...
				foreach ($d as $key => $value) {
					...
					//$item = $this->verify_itemid( $value["id"] );

					if ( $item = $this->verify_itemid( $value["id"] ) ) {
...
/*
		public function order_details($id, $r = false) {
			$stmt = $this->mysql->q("SELECT * FROM orders WHERE system_id = '".$id."'");
			...
			switch ($this->type) {
				...
				case 'addtobasket':
					...
					if ( $this->verify_data_switch() != false ) {
						...
						"addbutton" => $this->GD->generate_addtobasket( $this->verify_itemid( $this->data->basketitemid ), $this->basket) );
					} else {
						$r = array(
						"addbutton" => $this->GD->generate_addtobasket( $this->verify_itemid( $this->data->basketitemid, $_SERVER["HTTP_REFERER"] ), $this->basket) );
					}
				...
				case 'updatedetype':
					...
					if ( $delivery = $this->verify_detype( $this->data ) ) {
						$this->mysql->q("UPDATE basket SET shipping = '".$delivery->id."' WHERE basket = '". $this->basket->basket."'");

						//reset payment
						$this->mysql->q("UPDATE basket SET payment = NULL WHERE basket = '". $this->basket->basket."'");
				...
				//$this->order_data();
					if ( $this->basket->shipping ) {
						if ( $payment = $this->verify_payment( $this->data ) ) {
							$this->mysql->q("UPDATE basket SET payment = '".$payment->id."' WHERE basket = '". $this->basket->basket."'");
				...
				case 'company-tax':
					...
					//$stmt = $this->mysql->q("SELECT * FROM inputs WHERE idd = '$this->type'");
						...
						if ( !$dd = $this->check_delivery() ) {
							if ( $separated = $this->create_separated_data($data) ) {
								...
								if ( !$errors = $this->show_errors($rq) ) {
									...
									$r = array("error" => '<i class="fa fa-exclamation" aria-hidden="true"></i> '.$this->GD->text(584), "scroll" => "deliveryF", "errors" => $errors, "scroll" => "userdataF");
								}
							}
						} else {
							$r = array("error" => '<i class="fa fa-exclamation" aria-hidden="true"></i> '.$dd, "scroll" => "deliveryF");
						}
					} else {
						$r = array("error" => '<i class="fa fa-exclamation" aria-hidden="true"></i> '.$this->GD->random_text( array($this->GD->text(580), $this->GD->text(581), $this->GD->text(582)) ), "scroll" => "basketF");
					}
				...
				case "sendorder":
					...
					//$this->send_order($this->basket);

					if ( $r = $this->send_order($this->basket) ) {

						$this->send_mail_about_order($r);
		...
		public function generate_ordercomplete($id, $r = '') {
			...
			<div class="purchased">
				<div class="purchased-content">
					<div class="purchased-head"><i class="fa fa-check-circle" aria-hidden="true"></i><h2><span>'.$this->GD->text(599).'</span></h2></div>
		...
		public function create_separated_data($data, $r='') {
			foreach ($data as $key => $value) {
				...
				$stmt = $this->mysql->q("SELECT * FROM inputs WHERE idd = '$key'");
		...
		public function check_delivery($r = '') {
			...
			if ( !$this->basket->shipping /*!$this->verify_detype($this->basket->shipping*/ ) {
				$r = $this->GD->random_text( array($this->GD->text(578), $this->GD->text(583)) );
			} else {
				if ( !$this->basket->payment /*!$this->verify_payment($this->basket->payment)*/) {
					$r = $this->GD->text(579);
				}
			}
		...
		public function verify_data_switch($r = false) {
			...
			foreach ($this->data as $key => $value) {
				switch ($key) {
					...
					case 'basketitemid':
						$r = $this->verify_itemid( $this->data->basketitemid );
						break;
		...
		public function save_basket($data, $r='') {
			...
			if ( $data ) {
				foreach ($data as $key => $value) {
					...
					if ( $value ) {
						$this->mysql->query("UPDATE basket SET $set = '".$this->edit_value($set, $value)."' WHERE id = ".$this->basket->id);
						$r = true;
					} else {
						$this->mysql->query("UPDATE basket SET $set = '' WHERE id = ".$this->basket->id);
					}
				}
			}
		...
		public function send_order($data, $r = '') {
			...
			if ( $data ) {
				...
				$stmt = $this->mysql->q("SELECT system_id FROM orders WHERE system_id = '".$ID."'");
				...
				if ( $this->mysql->q("INSERT INTO orders (id, system_id) VALUES (NULL, '$new')") ) {
					
					$date = time();

					foreach ($data as $key => $value) {
						if ($key != "id" )
							$this->mysql->q("UPDATE orders SET $key = '".$value."' WHERE system_id = '".$new."'");
					}
					...
					$this->mysql->q("INSERT INTO orders_timeline (id, system_id, date, type) VALUES (NULL, '$new', '$date', '1')");
		...
		public function verify_itemid($id, $referer = "", $r = false) {
			...
			if ( $referer ) {
				foreach ( array_filter( explode("/", $referer) ) as $key => $value) {
					$stmt = $this->mysql->q("SELECT $select FROM navody WHERE public = 1 AND url = '".$value."'");
				...
				$stmt = $this->mysql->q("SELECT $select FROM navody WHERE public = 1 AND id = ".$id);
		...
		public function verify_quantity($r = false) {
			...
			$stmt = $this->mysql->q("SELECT quantity FROM navody WHERE public = 1 AND id = ".$this->data->basketitemid);
		...
		public function verify_price($r = false) {
			...
			$stmt = $this->mysql->q("SELECT price, discount FROM navody WHERE public = 1 AND id = ".$this->data->basketitemid);
		...
		public function verify_detype($id, $r = false) {
			$stmt = $this->mysql->q("SELECT * FROM delivery_type WHERE id = ".$id);
		...
		public function verify_payment($id, $r = false) {
			$stmt = $this->mysql->q("SELECT * FROM delivery_payment WHERE id = ".$id);
		...
		public function checkbasket($id, $r = '') {
			...
			$stmt = $this->mysql->q("SELECT * FROM basket WHERE basket = '".$id."'");

			if ( $r = $stmt->fetch_object() ) {
			} else {
				$this->mysql->q("INSERT INTO basket (id, basket) VALUES(NULL, '$id')");
			}
		...
		public function basket_update($r = '') {
			...
			if ( $data = $this->GD->basket_data( $this->basket ) ) {
				...
				//$this->mysql->q("UPDATE basket SET content = '".$r."' WHERE basket = '". $this->basket->basket."'");
			}
							...
							<div class="ba-quantity" data-maxquantity="'.$item->quantity.'">
								<div class="baq-c">
									...
									<div class="mobile-quantity">
										<button href="#" type="button" class="select-switcher showw" data-target=".select-'.$item->id.'"><span class="ii iDown2"></span></button>
										
										<div class="selectdata select-'.$item->id.'" data-target="#basketquantity-'.$item->id.'">
											<div class="selecthide"><button href="#" type="button" class="select-switcher" data-target=".select-'.$item->id.'"><span class="ii iUp2"></span></button></div>
							...
							<div class="ba-item">
								...
								<div class="ba-details">
									...
									<div class="ba-data">
										...
										<div class="ba-perone">'.$this->GD->price( $item->price ).'</div>
		...
		public function cart_summary($value='') {
		...
		public function shipping_data($table, $select, $r = 0) {
			...
			if ( $select ) {
				$stmt = $this->mysql->q("SELECT * FROM $table WHERE id = ".$select);
		...
		/*public function generate_inputs($form, $values = false, $r='') {
			...
			$stmt = $this->mysql->q("SELECT * FROM inputs WHERE form = ".$form." ORDER BY formpos ASC");
		...
		public function select_delivery_type($r='') {
			...
			$stmt = $this->mysql->q("SELECT * FROM delivery_type WHERE active = 1");
		...
		public function select_delivery_payment($r='') {
			//$key = 0;
			if ( $this->basket->shipping ) {
				$stmt = $this->mysql->q("SELECT * FROM delivery_type WHERE id = ".$this->basket->shipping);
			...
			$stmt = $this->mysql->q("SELECT * FROM delivery_payment WHERE active = 1");
...
/*
		public function shipping_payments ($type, $r='') {
			$stmt = $this->mysql->q("SELECT * FROM delivery_type WHERE id = ".$type);
		}
*/
		...
		public function generate_adresss ($r='') {
			...
			if ( $this->basket->delivery_firstname && $this->basket->delivery_lastname && $this->basket->delivery_phone && $this->basket->delivery_phone && $this->basket->delivery_street && $this->basket->delivery_city && $this->basket->delivery_zip ) {
				...
				<div class="och-adress deliAdress">
					<div class="ocha-content">
						<div class="ocha-head"><i class="fa fa-map-marker" aria-hidden="true"></i>'.$this->GD->text(550).'</div>
						<div class="ocha-body">
							<p>'.$billname.'</p>
							...
							<div class="ocha-contact">
								<p><i class="fa fa-phone" aria-hidden="true"></i>'.$this->basket->delivery_phone.'</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="och-adress deliAdress">
					<div class="ocha-content">
						...
						<div class="ocha-body">
							...
							<div class="ocha-contact">
								<p><i class="fa fa-phone" aria-hidden="true"></i>'.$this->basket->billing_phone.'</p>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="och-adress billAdress">
				<div class="ocha-content">
					<div class="ocha-head"><i class="fa fa-thumb-tack" aria-hidden="true"></i>'.$this->GD->text(586).'</div>
					<div class="ocha-body">
						<p>'.$billname.'</p>
						<p>'.$this->basket->billing_street.'</p>
						<p>'.$this->basket->billing_zip.' '.$this->basket->billing_city.'</p>
						...
						<div class="ocha-contact">
							<p><i class="fa fa-phone" aria-hidden="true"></i>'.$this->basket->billing_phone.'</p>
							<p><i class="fa fa-envelope-o" aria-hidden="true"></i>'.$this->basket->billing_email.'</p>
						</div>
					</div>
				</div>
			</div>
		...
		public function send_mail_about_order($id) {
			...
			$delivery = $this->verify_detype( $data->shipping );
			...
			$payment = $this->verify_payment( $data->payment );
