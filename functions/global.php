...
*/
		...
		public function category_data($id, $select) {
			$stmt = $this->m->q("SELECT $select FROM kategorie WHERE category = ".$id);
			$r = $stmt->fetch_assoc();
		...
		public function hashtag_data_by_id($id) {
			$stmt = $this->m->q("SELECT * FROM tagy WHERE id = '$id'");
...
*/
		...
		public function generate_availability($r="") {
			...
			$this->m->q("SELECT order, text FROM availability");
			
			foreach ( $this->m->resultO() as $v) { 
				$text = $this->text( $v->text );

				if ( $v->order == 1 ) {
					$r .= '<a href="#" class="edit-input l" name="'.$text.'" data-inputtarget="additem-availability">'.$text.'</a>';
				} else {
					$r .= '<a href="#" class="edit-input l gee1" name="'.$text.'" data-inputtarget="additem-availability">'.$text.'</a>';
				}
			}						
		...
		/*public function pagedata($pageID = "") {
			...
			$stmt = $this->m->q("SELECT id FROM pages WHERE ".$select." AND lang = '".$this->CHR->LANG."'");
		...
		public function generate_pictureUrl($data, $first = false, $AJAX = false, $r='') {
			$select = 'folder, filename, file_type, filename_orig';

			if ( $first == true ) {		
				$fir = explode("#", $data);

				if ( $firsst = array_filter($fir) ) {
					$stmt = $this->m->q("SELECT $select FROM files WHERE id = '".$firsst[1]."'");
				...
				$stmt = $this->m->q("SELECT $select FROM files WHERE id = ".$data);
...
*/
		...
		public function about_menu($type, $ul = true, $r = '') {
			...
			foreach ($about as $key => $value) {
				...
				if ( $page = $this->CHR->PAGEDATA_TARGET($value["page"]) ) {
		...
		// SEASONS
			...
			public function gen_seasons($r = '') {
				$c = 0;

				$stmt = $this->m->q("SELECT text, imgs, icon FROM seasons WHERE active = '1'");

				if ( $season = $stmt->fetch_object() ) {

					$data = $this->season_imgs($season->imgs);
					$cc = count($data);
					...
					foreach ( $data as $key => $v) {
						...
						$imgLink = $this->generate_pictureUrl( $v['img'], false );
						$imgSize = $this->picture_dimension( $imgLink["url"], $imgLink["url_nohhtp"] );

						$tag = $this->hashtag_data_by_id( $key );
						...
						$text = $tag->season_text ? $tag->season_text : $tag->id_text;

						$r .= '
						<div class="hs'.$pos.'">
							<a href="'.$this->link(8).'/'.$tag->sk.'">
								<div class="hs-body">
									<img src="'.$imgLink['url'].'" class="img'.$imgSize.'">
								</div>

								<div class="hs-head">
									<div class="ribbon"><div class="rib-text"><span class="rib rib-left"><span></span></span><h3 class="nowrap">'.$this->text( $text ).'</h3><span class="rib rib-right"><span></span></span></div></div>
								</div>
							</a>
						</div>
						';
					...
					$header = $season->text;
					$iconi = isset($season->icon) ? '<div class="hs-icon"><span class="ii '.$season->icon.'"></span></div>' : '';
				}

				if ( $r ) {

					return '
					<div class="home-seasons">
						<div class="hs-content hs4">
							<div class="hs-header">
								<div class="ribbon">
									<div class="rib-text">
										<span class="rib rib-left"><span></span></span><h3 class="nowrap">'.$this->text( 897 ).'</h3><span class="rib rib-right"><span></span></span>
										'.$iconi.'
									</div>
									<p>'.$this->text( $header ).'</p>
								</div>
			...
			public function season_imgs($imgs, $r ='') {
				
				if ( $imgs ) {

					foreach ( array_filter( explode('#', $imgs) ) as $key1 => $bytag) {
						
						$data = explode('*', $bytag);

						
						$stmt = $this->m->q("SELECT id FROM files WHERE id = ".$data[1]);

						if ( $file = $stmt->fetch_object() ) {
							$r[ $data[0] ] = array('img' => $data[1]);
						}
					}
				}
...
/*
			public function season_header($head, $r ='') {
				
				if ( $head ) {

					foreach ( array_filter( explode('#', $head) ) as $key => $v) {
						
						$stmt = $this->m->q("SELECT text FROM kategorie WHERE category = ".$v);

						if ( $cat = $stmt->fetch_object() ) {
							$r .= $this->text( $cat->text );
						}
					}
				}
			...
			public function gen_unavailable($r = '') {
				$c = 0;
				$all = 8;

				$stmt = $this->m->q("SELECT url, title, files, file_list FROM navody WHERE public = '1' AND availability = '8' LIMIT $all");

				if ( $data = $this->m->resultO() ) {

					foreach ($data as $key => $v) {
						$img = $v->file_list ? $v->file_list : $v->files;

						$imgLink = $this->generate_pictureUrl( $img, true );
						$imgSize = $this->picture_dimension( $imgLink["url"], $imgLink["url_nohhtp"] );
						...
						<div class="hun">
							<div class="hun-content">
								<a href="'.$this->url( $v->url ).'" class="hun-link" title="'.$v->title.'"><img src="'.$imgLink['url'].'" class="img'.$imgSize.'" alt="'.$v->title.'"></a>
							</div>
						</div>
						';
					}

					$empty = $all - count($data);

					if ( $empty > 0 ) {
						for ($i=0; $i < $empty; $i++) { 
							$r .= '<div class="hunEmpty"><div></div></div>';
						}
					}
				}
				...
				if ( $r ) {
					...
					return '
					<div class="home-unavailable">
						<div class="hs-content">
							<div class="hs-header">
								<div class="ribbon"><div class="rib-text"><span class="rib rib-left"><span></span></span><h3 class="nowrap">'.$this->text( 899 ).'</h3><span class="rib rib-right"><span></span></span></div></div>
							</div>
