<?php
	...
	class BASKET {
		...
		public function __construct($type = "") {
			...
			if ( $this->basket = $this->checkbasket() )
				$this->order_data();
...
*/
		...
		public function checkbasket($r = '') {
			$this->basketid = isset($_COOKIE["mm-uid"]) ? $_COOKIE["mm-uid"] : $this->GD->uid();

			$stmt = $this->m->q("SELECT * FROM basket WHERE basket = '".$this->basketid."'");

			if ( $r = $stmt->fetch_object() ) {
			} else {
				$this->m->q("INSERT INTO basket (id, basket) VALUES(NULL, '$this->basketid')");
		...
		public function gen_basket($r = '') {
			...
			if ( !empty($this->basket) ) {
				$c = count( array_filter( explode("#", $this->basket->content ) ) );
